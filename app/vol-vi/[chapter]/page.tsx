...
export default async function ChapterPage({ params }: Props) {
  ...
  const filePath = path.join(process.cwd(), 'content', 'vol-vi', `${chapter}.mdx`)
  ...
  const { prev, next } = getAdjacentChapters(chapter, 'vol-vi')
  ...
  return (
    <>
      ...
      />
      <PageNav prev={prev} next={next} basePath="vol-vi" />
    </>
  )
}
